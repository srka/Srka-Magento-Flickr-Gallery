...
class Srka_Flickrgallery_Block_Photosets extends Srka_Flickrgallery_Block_View {
	...
	public function getPhotosets(){
		return (!empty($this->_raw_photosets) && is_array($this->_raw_photosets) && (array_key_exists('sets', $this->_raw_photosets)) ? $this->_raw_photosets['sets'] : array() );
	}

	public function getPhotosetId($photoset){
		return (!empty($photoset) && is_array($photoset) && array_key_exists('id', $photoset)) ? $photoset['id'] : '';
	}

	public function getPhotosetTitle($photoset){
		return (!empty($photoset) && is_array($photoset) && array_key_exists('title', $photoset)) ? $photoset['title'] : '';
	}

	public function getPhotosetDescription($photoset){
		return (!empty($photoset) && is_array($photoset) && array_key_exists('description', $photoset)) ? $photoset['description'] : '';
	}

	public function getPhotosetUrl($photoset){
		return (!empty($photoset) && is_array($photoset) && array_key_exists('id', $photoset)) ? Mage::getUrl('gallery/set/id/') . $photoset['id'] : '';
	}
